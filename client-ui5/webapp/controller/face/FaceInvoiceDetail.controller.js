sap.ui.define([
    ...
    return Controller.extend("invoice.app.controller.face.FaceInvoiceDetail", {
        formatter: formatter,

        onInit: function () {
            var oRouter = this.getOwnerComponent().getRouter();
            oRouter.getRoute("faceInvoiceDetail").attachPatternMatched(this._onObjectMatched, this);
        ...
        _loadInvoiceDetail: function (sInvoiceId) {
            ...
            Promise.all([
                ...
                fetch("/api/face/invoices/impuestos", {
                    headers: { "Authorization": "Bearer " + sToken }
                }).then(r => r.json())
            ])
        ...
        onChangeStatus: function (oEvent) {
            ...
            fetch("/api/face/invoices/facturas/" + sInvoiceId + "/estado", {
                method: "PATCH",
                headers: {
        ...
        onViewXML: function () {
            var oModel = this.getView().getModel("detail");
            var sInvoiceId = oModel.getProperty("/id_factura");
            this.getOwnerComponent().getRouter().navTo("faceFacturaE", {
        ...
        onViewPDF: function () {
            ...
            var sInvoiceId = oModel.getProperty("/id_factura");
            var sToken = localStorage.getItem("auth_token");
            var sUrl = "/api/face/invoices/facturas/" + sInvoiceId + "/pdf";
        ...
        onGenerateUBL: function () {
            var oModel = this.getView().getModel("detail");
            var sInvoiceId = oModel.getProperty("/id_factura");
            this.getOwnerComponent().getRouter().navTo("faceUbl21", {
                invoiceId: sInvoiceId
            });
        },
        ...
        onEdit: function () {
            var oModel = this.getView().getModel("detail");
            var sInvoiceId = oModel.getProperty("/id_factura");
            this.getOwnerComponent().getRouter().navTo("faceInvoiceEdit", {
                invoiceId: sInvoiceId
            });
        },
