sap.ui.define([
    ...
    return Controller.extend("invoice.app.controller.face.FaceInvoiceForm", {
        formatter: formatter,

        onInit: function () {
            var oRouter = this.getOwnerComponent().getRouter();
            var oRouteNew = oRouter.getRoute("faceInvoiceNew");
            if (oRouteNew) {
                oRouteNew.attachPatternMatched(this._onRouteMatched, this);
            } else {
                console.error("DEBUG: Route 'faceInvoiceNew' not found in router! Check manifest.json");
            }

            var oRouteEdit = oRouter.getRoute("faceInvoiceEdit");
            if (oRouteEdit) {
        ...
        _onRouteMatched: function (oEvent) {
            ...
            if (sRouteName === "faceInvoiceNew") {
                console.log("DEBUG: InvoiceForm - Route faceInvoiceNew matched");
                        ...
                        if (oModel) {
                            ...
                            if (oFoundReceptor) {
                                ...
                                if (oModel.getProperty("/codigo_tipo") === "02" && aReceptores.length > 0) {
                                    ...
                                    sap.m.MessageToast.show("Receptor auto-seleccionado: " + aReceptores[0].nombre);
                                }
                            }
        ...
        onViewJson: function () {
            ...
            if (!oJson) {
                MessageToast.show("No hay datos JSON disponibles");
                return;
            }
        ...
        onCancel: function () {
            if (this._bFromScan) {
                this.getOwnerComponent().getRouter().navTo("faceScanInvoice");
            } else {
                this.onNavBack();
            }
        },
        ...
        onClientSelect: function (oEvent) {
            ...
            // We just look it up.
            // Note: In _loadFormData we perform fetch("/api/face/invoices/receptores"), so 'receptores' is available.
        ...
        onGenerateNumber: function () {
            ...
            fetch("/api/face/invoices/facturas/next-number", {
                ...
                .catch(err => {
                    ...
                    sap.m.MessageToast.show("Error generando número de factura");
                });
        },
        ...
        _loadFormData: function () {
            ...
            return Promise.all([
                fetch("/api/face/invoices/emisores", {
                    headers: { "Authorization": "Bearer " + sToken }
                }).then(r => r.json()),
                fetch("/api/face/invoices/receptores", {
                    headers: { "Authorization": "Bearer " + sToken }
                }).then(r => r.json()),
                fetch("/api/face/invoices/impuestos?activo=true", {
                    headers: { "Authorization": "Bearer " + sToken }
                }).then(r => r.json()),
                ...
                MessageToast.show("Error cargando datos del formulario");
            });
        },
        ...
        _loadInvoiceForEdit: function (sInvoiceId) {
            ...
            fetch("/api/face/invoices/facturas/" + sInvoiceId, {
                ...
                .catch(function (error) {
                    ...
                    MessageToast.show("Error cargando factura");
                });
        },
        ...
        onAddFromCatalog: function () {
            ...
            this._pCatalogDialog.then(function (oDialog) {
                ...
                if (!oCatalogModel) {
                    ...
                    var sToken = localStorage.getItem("auth_token");
                    fetch("/api/face/catalog/products?lang=" + sCurrentLang, {
                        headers: { "Authorization": "Bearer " + sToken }
                    })
                        .then(res => res.json())
        ...
        onDeleteLine: function (oEvent) {
            ...
            if (aLineas.length > 1) {
                ...
                MessageToast.show("Se requiere al menos una línea");
            }
        },
        ...
        onSave: function () {
            ...
            if (missingFields.length > 0) {
                ...
                MessageBox.error("Por favor rellene todos los campos requeridos: " + missingFields.join(", "));
                return;
            }

            if (oData.lineas.length === 0) {
                MessageBox.error("Se requiere al menos una línea");
                return;
            }
            ...
            var sUrl = oData.isEdit
                ? "/api/face/invoices/facturas/" + oData.id_factura
                : "/api/face/invoices/facturas";
            var sMethod = oData.isEdit ? "PUT" : "POST";

            // Client-side validation
            if (!oData.id_emisor || oData.id_emisor === "") {
                MessageBox.error("Falta el Emisor. Por favor seleccione uno de la lista.");
                return;
            }
            if (!oData.id_receptor || oData.id_receptor === "") {
                MessageBox.error("Falta el Receptor. Por favor seleccione uno de la lista.");
                return;
            }
            ...
            fetch(sUrl, {
                ...
                .then(async function (response) {
                    if (response.ok) {
                        ...
                        throw new Error(errData.message || "Error saving invoice");
                    }
                })
                .then(function (data) {
                    MessageToast.show("Factura guardada correctamente");
                    that.onNavBack();
                })
                .catch(function (error) {
                    console.error("Error saving invoice:", error);
                    MessageBox.error("Error guardando factura: " + error.message);
                });
        },
